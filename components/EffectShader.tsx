import React, { forwardRef, useMemo } from 'react';
import { Uniform } from 'three';
import { Effect } from 'postprocessing';

// Fragment shader that implements an ordered dithering (Bayer matrix)
// to reduce the image to 1-bit (black and white)
const fragmentShader = `
  uniform float opacity;
  uniform vec2 resolution;

  const float bayerMatrix8x8[64] = float[](
    0.0/64.0, 48.0/64.0, 12.0/64.0, 60.0/64.0,  3.0/64.0, 51.0/64.0, 15.0/64.0, 63.0/64.0,
    32.0/64.0, 16.0/64.0, 44.0/64.0, 28.0/64.0, 35.0/64.0, 19.0/64.0, 47.0/64.0, 31.0/64.0,
    8.0/64.0, 56.0/64.0,  4.0/64.0, 52.0/64.0, 11.0/64.0, 59.0/64.0,  7.0/64.0, 55.0/64.0,
    40.0/64.0, 24.0/64.0, 36.0/64.0, 20.0/64.0, 43.0/64.0, 27.0/64.0, 39.0/64.0, 23.0/64.0,
    2.0/64.0, 50.0/64.0, 14.0/64.0, 62.0/64.0,  1.0/64.0, 49.0/64.0, 13.0/64.0, 61.0/64.0,
    34.0/64.0, 18.0/64.0, 46.0/64.0, 30.0/64.0, 33.0/64.0, 17.0/64.0, 45.0/64.0, 29.0/64.0,
    10.0/64.0, 58.0/64.0,  6.0/64.0, 54.0/64.0,  9.0/64.0, 57.0/64.0,  5.0/64.0, 53.0/64.0,
    42.0/64.0, 26.0/64.0, 38.0/64.0, 22.0/64.0, 41.0/64.0, 25.0/64.0, 37.0/64.0, 21.0/64.0
  );

  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    // Convert to grayscale using standard luminance weights
    float lum = dot(inputColor.rgb, vec3(0.299, 0.587, 0.114));

    // Calculate pixel coordinates
    vec2 xy = gl_FragCoord.xy;
    int x = int(mod(xy.x, 8.0));
    int y = int(mod(xy.y, 8.0));
    
    // Get threshold from Bayer matrix
    // We flip y index because gl_FragCoord is bottom-up
    int index = y * 8 + x;
    float threshold = bayerMatrix8x8[index];

    // 1-bit quantization
    vec3 color = (lum > threshold) ? vec3(1.0) : vec3(0.0);

    // Scanline effect (optional, subtle)
    // if (mod(xy.y, 2.0) < 1.0) color *= 0.9;

    outputColor = vec4(color, inputColor.a);
  }
`;

class DitherEffectImpl extends Effect {
  constructor() {
    super('DitherEffect', fragmentShader, {
      uniforms: new Map([
        ['resolution', new Uniform(new Float32Array([window.innerWidth, window.innerHeight]))]
      ]),
    });
  }
}

export const DitherEffect = forwardRef(({ }, ref) => {
  const effect = useMemo(() => new DitherEffectImpl(), []);
  return <primitive object={effect} ref={ref} dispose={null} />;
});
